...
def analyze_3d_array_structure():
    ...
    try:
        # 데이터 로드 (디렉토리)
        data_source = TRAIN_DATA_DIR
        
        if not data_source:
            raise ValueError("TRAIN_DATA_DIR must be configured in config.py")
        ...
        print(f"✓ 특성(X): {len(feature_cols)}, 레이블(y): {label_col}")
...
def create_visualization(X, y):
    ...
    try:
        ...
        # 저장
        output_path = PROJECT_ROOT / "analysis" / "3d_array_visualization.png"
