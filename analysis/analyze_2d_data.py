...
def load_and_preprocess_data():
    ...
    try:
        data_source = TRAIN_DATA_DIR if TRAIN_DATA_DIR else None
        
        if not data_source:
            raise ValueError("TRAIN_DATA_DIR not configured")
        
        data_loader = DataLoader(data_path=str(data_source))
        
        df = data_loader.load_data()
        analysis = data_loader.analyze_columns()
...
def create_visualizations(X_2d, y, X_3d, pca_components_data):
    ...
    cumsum_var = np.cumsum(pca.explained_variance_ratio_)
    max_components = min(50, len(cumsum_var))
    ax3.plot(range(1, max_components + 1), 
             cumsum_var[:max_components], 'o-', linewidth=2)
    ...
    # 저장
    output_path = PROJECT_ROOT / "analysis" / "ml_2d_analysis_visualization.png"
